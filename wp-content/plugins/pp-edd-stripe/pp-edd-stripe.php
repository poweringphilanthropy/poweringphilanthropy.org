...
class EDD_Stripe {
	...
	private function actions() {
		...
		// remove_action( 'edd_purchase_form_after_cc_form', 'edd_checkout_submit' );
		add_action( 'edd_purchase_form_after_user_info',  array( self::$instance,'edd_purchase_form_after_user_info_new'), 10 );
		add_action( 'edd_register_fields_before',  array( self::$instance,'edd_purchase_form_after_user_info_new'), 10 );
	}
	...
	if((isset($_POST['donor_selection']) && $_POST['donor_selection'] == "true") || !isset($_POST['donor_selection'])){
		...
		public function edd_purchase_form_after_user_info_new(){
			// exit;
			...
			?>
			<!-- <div id="edd_checkout_user_info 22">-->
			
				
				
				<div class="col-lg-6 form-spacing">
					<div class="form-group" id="edd-first-name-wrap">
						<label class="form-check-label text-left text-uppercase  mb-1" for="inlineCheckbox1 ">
							<?php esc_html_e( 'First Name', 'easy-digital-downloads' ); ?>
							<?php if( edd_field_is_required( 'edd_first' ) ) { ?>
								<span class="edd-required-indicator">*</span>
							<?php } ?>
						</label>
						<input class="edd-input required form-control" type="text" name="edd_first"  id="edd-first" value="<?php echo esc_attr( $customer['first_name'] ); ?>"<?php if( edd_field_is_required( 'edd_first' ) ) {  echo ' required '; } ?> aria-describedby="edd-first-description" />
					</div>
				</div>
				...
				<div class="col-lg-6 form-spacing">
					<div class="form-group">
						...
						<input class="edd-input form-control <?php if( edd_field_is_required( 'edd_last' ) ) { echo ' required'; } ?>" type="text" name="edd_last" id="edd-last"  value="<?php echo esc_attr( $customer['last_name'] ); ?>"<?php if( edd_field_is_required( 'edd_last' ) ) {  echo ' required '; } ?> aria-describedby="edd-last-description"/>
					</div>
				</div>			
				...
				<div class="col-lg-12 form-spacing">
					<div class="form-group ">
						...
						<input class="edd-input required form-control" type="email" name="edd_email"  id="edd-email" value="<?php echo esc_attr( $customer['email'] ); ?>" aria-describedby="edd-email-description"<?php if( edd_field_is_required( 'edd_email' ) ) {  echo ' required '; } ?>/>
				...
				<br>
					...
					<div class="form-check">
						<div class="select-type col-lg-6">
						  <label><input name="display" type="checkbox" class="subject-list" value="fulname">FULL NAME</label>
						</div>
						<div class="select-type col-lg-6">
						  <label><input name="display" type="checkbox" class="subject-list" value="anonymous"> ANONYMOUS</label>
						</div>
					</div>
				</div>
				<div class="col-lg-12 form-spacing">
					<div class="form-check">
						<div class="select-type col-lg-2">
						  <label><input name="display" type="checkbox" class="subject-list" value="other"> OTHER:</label>
						</div>
						<div class="select-type col-lg-10">
						  <input type="text" class="form-control" name="other_name" id="other_name" placeholder=" ">
						</div>
					</div>
				</div>
