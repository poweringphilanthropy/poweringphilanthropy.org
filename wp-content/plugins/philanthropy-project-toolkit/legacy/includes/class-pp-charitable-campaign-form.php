...
if (!class_exists('PP_Charitable_Campaign_Form')) :
    ...
    class PP_Charitable_Campaign_Form {
        ...
        protected function attach_hooks_and_filters() {
            ...
            add_action( 'charitable_campaign_submission_save', array( $this, 'save_give_my_day' ), 10, 3 );
		...
		public function save_give_my_day($submitted, $campaign_id, $user_id){
			
			
            if(isset($submitted['media_video']) ){
                update_post_meta( $campaign_id, 'media_video', $submitted['media_video']);
            } 
			
			if(isset($submitted['in_support']) && !empty($submitted['in_support']) ){
                update_post_meta( $campaign_id, 'in_support', $submitted['in_support']);
            }

			if(isset($submitted['charitable_tax_id']) && !empty($submitted['charitable_tax_id']) ) {
				$str1 = substr($submitted['charitable_tax_id'], 0, 2);
				$str3 = substr($submitted['charitable_tax_id'], 2);
				$str2 = "-";
				$output = "$str1$str2$str3";
				$mydb = new wpdb('poweri33_wp872','f]1P1SL!1p','poweri33_wp872','localhost');
				$row = $mydb->get_row("select * from wpv7_nonprofit_organizations WHERE tax_id='".$output."' ");
				$count = count($row);
				if($count == 1){
					   update_post_meta( $campaign_id, '_campaign_payout_options', "direct");
					   update_post_meta( $campaign_id, '_campaign_payout_option_type', "direct");
					   update_post_meta( $campaign_id, '_campaign_connected_stripe_id', $row->stripe_id);
					   update_post_meta( $campaign_id, 'charitable_ein_number', $submitted['charitable_tax_id']);
        ...
        public function custom_pp_toolkit_sections($fields, Charitable_Ambassadors_Campaign_Form $form){
            ...
            $fields = apply_filters( 'pp_toolkit_campaign_submission_fields', array(
				'profile_fields' => array(
                    'legend'        => __( 'About You', 'pp-toolkit' ),
                    // 'things_to_know'        => 'Details about section here',
                    'type'          => 'fieldset',
                    'fields'        => $this->get_account_options_fields($form),
                    'priority'      => 65,
                    'page'          => 'campaign_details',
                    'hide_header'   => true,
                ),
                'campaign_fields' => array(
                    'legend'        => __( 'Basic Info', 'pp-toolkit' ),
                    // 'things_to_know'        => 'Details about section here',
                    'type'          => 'fieldset',
                ...
                'fundraising_fields' => array(
                    'legend'        => __( 'Personalize your Fundraiser', 'pp-toolkit' ),
                    // 'things_to_know'        => 'Details about section here',
                    'type'          => 'fieldset',
        ...
        public function get_campaign_fields(Charitable_Ambassadors_Campaign_Form $form){
			...
			$uriSegments[2];
			if( $uriSegments[2] == "" ){
			//	echo "uriSegments==>".$uriSegments[2];
				$fields['in_support'] = [
					'priority'      => 2,
					'label'         => __('Choose My Charity', 'pp-toolkit'),
					'type'          => 'text',
					'required'      => true,
					'fullwidth'     => false,
					'value'           => $this->get_campaign_value('in_support'),
					'value'         => $campaign_id,
					'data_type'     => 'meta',
					'id'     		=> 'in_support_of',
				...
				$fields['in_support_msg'] = [
			///	'label'         => __('Choose My Charity', 'pp-toolkit'),
					'priority'      => 2,
					'type'          => 'paragraph',
					//'content'   => 'Your charity has already been selected. Email hello@givemyday.com if you need to make a change',
					'content'   => '<div id="charitable_field_in_support" class="charitable-form-field charitable-form-field-text required-field even"><label for="charitable_field_in_support_element">Choose My Charity <abbr class="required" title="required">*</abbr></label><p>Your charity has already been selected. Email <a href="mailto:hello@givemyday.com">hello@givemyday.com</a>  if you need to make a change.</p></div>',
					'fullwidth'     => false,
					'class'   => 'in_support_msg'
				];
			}
			
			
			$fields['charitable_tax_id'] = [
                'priority'      => 6,
                'label'         => __('Tax ID', 'pp-toolkit'),
                'type'          => 'hidden',
                'required'      => false,
                'fullwidth'     => false,
                'data_type'     => 'meta',
                'class'     => 'charitable_tax_id',
                'editable'      => true
            ];
            ...
            if(isset($fields['description'])){
                ...
                $fields['description']['placeholder'] = "In a sentence or two, tell people why you are giving your day, which day you are giving & which organization you would like to support.";
            }
        ...
        public function get_fundraising_options_fields(Charitable_Ambassadors_Campaign_Form $form){
            ...
            $fundraising_fields = array(
                'suggested_donations' => [
                    ...
                    'priority' => 60,
                    'page'     => 'campaign_details',
                    'label'     => 'Additional Details About Your Philanthropy',
                    'value'     => '',
			...
			'additional_details' => [
				...
				'image' => [
                    ...
                    'type'     => 'image-crop',
                    'priority' => 50,
					'value' => $this->get_field_value('image'),
					'label'     => __('Featured Image (.jpg or .png)', 'pp-toolkit'),
                    'page'     => 'campaign_details',
                    'fullwidth'     => false,
		...
		//	media_video
				...
				'media_video' => [
                    ...
                    'type'     => 'textarea',
                    'priority' => 51,
					'value'         => $this->get_campaign_value('media_video'),
					'label'     => __('Featured Video', 'pp-toolkit'),
                    'page'     => 'campaign_details',
					'placeholder' => 'Embed a featured video by entering the "share" URL from almost any source. (Examples: https://youtu.be/videoID or https://www.facebook.com/videoID)',
					'data_type' => 'meta',
                    'fullwidth'     => false,
				...
				'post_content' => [
					...
					'label'     => __('Why is this cause important to you?', 'pp-toolkit'),
                    'page'     => 'campaign_details',
                    'fullwidth'     => true,
					'editor-setting' => [
					   'tinymce ' => array(
        ...
        public function get_account_options_fields(){
			...
			$formclass = new Charitable_Profile_Form();
            $account_fields = array(
                'first_name' => array(
               ...
               'password_fields' => array(
					...
					'type'     => 'fieldset',
					'fields'   => $formclass->get_password_fields(), 
					'priority' => 60,
				),
            );
        ...
        public function campaign_core_date($values, $user_id, $submitted) {
            if (isset($submitted['post_date'])) {
                $values['post_date'] = current_time( 'mysql' );
                // $values['post_date'] = date('Y-m-d H:i:s');
