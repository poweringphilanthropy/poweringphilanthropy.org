...
class PP_Merchandise_Form extends Charitable_Form {
    ...
    public static function save_merchandise( $submitted, $campaign_id, $user_id ) {
        ...
        foreach ( $merchandise as $index => $product ) {
            ...
            if ( ! isset( $product[ 'campaign_benefactor_id' ] ) || 0 == $product[ 'campaign_benefactor_id' ] ) {
                ...
                $form->save_benefactor_relationship( $campaign_id, $product_id , false );

            }else{
				$form->save_benefactor_relationship( $campaign_id, $product_id , true );
			}
     ...
     * @since   1.0.0
     */
    public function save_benefactor_relationship( $campaign_id, $product_id , $is_update = false) {
        ...
        if (!$has_relationship) {
			...
			if(!$is_update){
				...
				return charitable()->get_db_table( 'benefactors' )->update( $product_id , $benefactor_data );
			}
        }
        return false;
    }    
