...
class PP_Ticket_Form extends Charitable_Form {
    ...
    public static function save_benefactor_relationships( $ticket_id, $event_id, $campaign_id ) {
        ...
        if (!$has_relationship) {
			...
			global $wpdb;
			$already_benefactors = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT campaign_benefactor_id
					FROM ".$wpdb->prefix ."charitable_edd_benefactors
					WHERE edd_download_id = %d order by campaign_benefactor_id DESC;",
					$ticket_id 
				),
				OBJECT_K
			);
			if(empty($already_benefactors)){
				charitable()->get_db_table( 'benefactors' )->insert( $benefactor_data );
			}else{
				$i = 0;
				foreach($already_benefactors as $benef_id){
					if($i == 0){ charitable()->get_db_table( 'benefactors' )->update($benef_id->campaign_benefactor_id , $benefactor_data );}else{charitable_get_table( 'benefactors' )->delete( $benef_id->campaign_benefactor_id );}
					$i++;
				};
